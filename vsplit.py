...
HELP = """
Usage: vsplit -h | -(opt) (value) ...

Options:
    -h      display this message
    -c1     command for top terminal (default: tty-clock -c -C 3)
    -c2     command for bottom terminal (default: cava)
...
class WidgetWindow(Gtk.Window):
    def __init__(self):
        ...
        term_top = self.create_term_top()
        term_bottom = self.create_term_bottom()

        overlay.add_overlay(term_top)
        overlay.add_overlay(term_bottom)

        self.show_all()

    def create_term_top(self):
        term = Vte.Terminal()

        term.set_size_request(1900, 507)
        term.set_halign(Gtk.Align.CENTER)
